...
doc), which is then scored, shortlisted, and ranked (`sql/06` through `sql/10`), with
`sql/11` building the two Tableau dashboard extracts on top. Built in Postgres; see the
inline comments in `sql/01` through `sql/11` for the SQL itself.
...
**Grain:** one row is one candidate program's closest-match BLS occupation (12 rows, one per program in `labor_raw/`).

| Column | dtype | Meaning | Nulls |
|---|---|---|---|
| `program` | object | Candidate program slug (matches a `labor_raw/*.md` filename) | 0/12 |
| `occupation_title` | object | Closest-match BLS occupation name (a proxy, not exact) | 0/12 |
| `median_pay` | object | 2024 median annual wage, formatted as `"$124,910"` (string, strip `$`/`,` before math) | 0/12 |
| `entry_level_education` | object | Typical entry education per BLS OOH; 3 distinct values including `"Varies by specialization (no single BLS answer)"` for occupations without one clean answer | 0/12 |
| `number_of_jobs_2024` | object | 2024 employment count, comma-formatted string (e.g. `"182,800"`) | 0/12 |
| `job_outlook_growth_pct` | int64 | BLS 2024 to 34 projected growth %, already numeric | 0/12 |
| `employment_change_2024_34` | object | Projected net employment change 2024 to 34, comma-formatted string | 0/12 |

---

### `bls_fastest_growing.csv`  (BLS Employment Projections Table 1.3)
**Grain:** one row is one occupation in BLS's pre-ranked fastest-growing list (31 rows: 30 ranked occupations plus 1 `"Total, all occupations"` header/baseline row; filter it out for ranking math).
...
**Grain:** one row is one occupation in BLS's pre-ranked "most new jobs" list (31 rows: 30 ranked plus 1 `"Total, all occupations"` header row, same convention as Table 1.3).
...
**Grain:** one row is one SOC occupation (or occupation-group rollup) with its 2024 to 34 separations/openings projections. 1,113 rows: 832 `"Line item"` (detail occupation) rows plus category-summary (`"Summary"`) rollup rows. Filter `Occupation type == "Line item"` before treating rows as independent detail occupations, per the source note in `data-sources.md`.
...
| `Total occupational separations rate,<br> 2024–34 annual average` | float64 | Exit rate plus transfer rate combined | 0/1113 |
...
| `Occupational openings,<br> 2024–34 annual average` | object | **The key labor-demand metric**: annual-average total openings (growth plus separations), comma-formatted string | 0/1113 |

Note: the literal `<br>` in five header names comes straight from the scraped HTML and needed cleanup (regex-stripping `<br>` and collapsing whitespace) during Phase 2 cleaning.

---

### `vanderbilt_current_programs.csv`
**Grain:** one row is one Vanderbilt Online Learning program (degree or certificate), 27 rows.

| Column | dtype | Meaning | Nulls |
|---|---|---|---|
| `program_name` | object | Full program title, unique per row | 0/27 |
| `school` | object | Owning school (11 distinct); one row has a dual-affiliation value `"School of Nursing \| Graduate School"`, pipe-delimited, worth normalizing before grouping by school | 0/27 |
...
**Grain:** one row is one CIP6 program code times year (Masters degree level only), 14,493 rows across 1,290 distinct CIP6 codes and years 2012 to 2024 (13 possible years).
...
| `Year` | int64 | Completion year, 2012 to 2024 | 0/14493 |
| `Completions` | float64 | National count of Masters completions for that CIP6/year | 0/14493 |

**Missing-data note:** no `NaN`s in any column, but coverage is row-suppressed rather than null-filled: 319 of 1,290 CIP6 codes (25%) have fewer than the full 13 years of rows (min 1, mean roughly 11.2), consistent with the IPEDS privacy-suppression flagged in `data-sources.md`. Absence of a CIP6/Year row means suppressed or zero; don't assume 0 when reindexing to a full year range.

---

### `ipeds_completions_masters_raw.json`
**Grain:** the unparsed API response `ipeds_completions_masters.csv` was derived from, one JSON object with a `data` array of 14,493 records (same grain as the CSV: one record is one CIP6 × Year).
...
| `data` | list[dict] | The actual 14,493 records, e.g. `{"CIP6 ID": 10000, "CIP6": "General Agriculture", "Year": 2012, "Completions": 316.0}`, which `scripts/parse_ipeds_completions.py` flattens into the CSV above |

---

### `cip_soc_crosswalk.xlsx`
NCES's official CIP2020↔SOC2018 crosswalk, distributed as one workbook with 8 sheets serving different lookup directions. This section documents the raw file as originally profiled; the cleaned version (`cip_soc_crosswalk_clean.csv`) is what `sql/04_build_derived_candidates.sql` actually joins on, per the Analysis marts section below.
...
Common columns across the CIP-keyed sheets: `CIP2020Code` (float64, e.g. `1.0101`; the decimal encodes what's conceptually a 6-digit code, so `1.0` and `1.0000` are not the same value, a float-precision join risk). `CIP2020Title` (object, titles end in a trailing period, e.g. `"Agriculture, General."`). Common columns across the SOC-keyed sheets: `SOC2018Code` (object, e.g. `"19-1011"`), `SOC2018Title` (object). Every sheet uses the sentinel code/title pair above for "no match," never a blank cell.
...
**Grain:** one row is one occupation (30 rows, same baseline-row drop as above).
...
**Grain:** one row is one detail SOC occupation (832 rows, filtered from the raw file's 1,113 to `Occupation type == "Line item"` only; the 281 category-rollup "Summary" rows were dropped).
...
| `total_separations_rate_2024_34` | float64 | Exit rate plus transfer rate combined | 0/832 |
...
| `occupational_openings_2024_34` | float64 | **The key labor-demand metric**, annual-average total openings (growth plus separations) | 0/832 |
...
**Grain:** one row is one CIP2020×SOC2018 match (6,097 rows, the raw `CIP-SOC` sheet only; the other 7 sheets in the raw `.xlsx` were not used). 2,143 distinct CIP codes, 868 distinct SOC codes, confirmed unchanged from the raw profile.
...
**Grain:** one row is one CIP6 program code times year, Masters level only (14,331 rows, down from the raw file's 14,493; the 22 CIP6 codes with no match anywhere in the cleaned crosswalk were dropped, taking distinct CIP6 codes from 1,290 to 1,268).

| Column | dtype | Meaning | Nulls |
|---|---|---|---|
| `cip6_id` | int64 | Numeric CIP6 code (e.g. `10000`), unchanged from raw | 0/14331 |
| `cip6_title` | object | CIP6 program name | 0/14331 |
| `year` | int64 | Completion year, 2012 to 2024 | 0/14331 |
| `completions` | float64 | National Masters completions count for that CIP6/year | 0/14331 |
| `cip2020_code` | float64 | Join key derived from `cip6_id` (zero-pad plus decimal insert, e.g. `10000` becomes `1.0`, matching the crosswalk's `cip2020_code` format) | 0/14331 |

Missing years (IPEDS suppression) are left missing, not filled with 0; suppression is a property of the source, not a cleaning step, and the raw-section note above still applies here.

### `vanderbilt_current_programs_clean.csv`
**Grain:** one row is one Vanderbilt online Master's or Doctorate program (15 rows, filtered from the raw file's 27; all 12 Certificate-level rows were dropped, since certificates require existing Vanderbilt enrollment and aren't standalone programs comparable to the Masters-level IPEDS data driving the rest of this project).
...
Built by `sql/04_build_derived_candidates.sql` (Postgres; full pipeline
`scripts/run_sql_pipeline.sh`). **Grain: one row per CIP6 candidate program, 1,268
rows, PK-enforced.** Combines student demand (IPEDS) and labor demand (BLS via the
crosswalk, locked top-3-SOC employment-weighted rule). Scoring is layered on top in
...
| `completions_trend_pct` | NUMERIC | 126 | % change in completions, earliest to latest available year. NULL when only one year exists or the base year is 0, meaning unknown rather than 0% |
| `matched_soc_codes` | TEXT | 77 | The CIP's top-3 or fewer matched SOCs by 2024 employment, comma-separated, biggest first |
| `n_socs_used` | INTEGER | 0 | How many SOCs back the weighted metrics (0 to 3; 0 means no crosswalk match with BLS data) |
| `employment_weighted_openings` | NUMERIC | 77 | Avg annual occupational openings 2024 to 34 (thousands), employment-weighted across the top-3 SOCs |
| `employment_weighted_growth_pct` | NUMERIC | 77 | Projected employment growth % 2024 to 34, same weighting |
| `in_bls_top30_flag` | BOOLEAN | 0 | TRUE if any top-3 SOC appears on BLS's fastest-growing or most-new-jobs top-30 lists (a wage-adjacent signal; wages themselves are never blended in, since only 60/832 SOCs have wage data) |
| `already_offered_by_vanderbilt` | BOOLEAN | 0 | Exact CIP6 match against VU's **14 distinct** codes (15 programs; two M.Ed.s legitimately share 13.0401 per the Registrar PDF). Exactly 14 rows TRUE; rows are flagged, never dropped |

### `scored_candidates` (`data/derived/scored_candidates.csv`)

Built by `sql/06_score_candidates.sql` (band column added by `sql/08_score_bands.sql`).
**Grain: one row per CIP6 candidate program, 1,268 rows, same grain and PK as
`derived_candidates`.** Turns `derived_candidates`'s four continuous metrics into 0 to
100 percentile ranks, combines them via the locked 30/30/15/15/10 weighted average
(partial reweight when a metric is missing), and bands the result into Go/Test/Pass.
...
| `pct_completions_latest_year` | NUMERIC | 0 | Percentile rank (0 to 100) on completions level |
...
| `n_metrics_used` | INTEGER | 0 | How many of the 5 weighted components had real data for this row (range 2 to 5); the denominator partial-reweight rescales against |
| `score` | NUMERIC | 0 | 0 to 100 composite score, `weighted_sum / weight_used`, rounded to 1 decimal |
...
Built by `sql/09_shortlist_review.sql`, joining `scored_candidates`'s Go band back to
`derived_candidates` for SOC-level detail. **Grain: one row per Go-band candidate, 126
rows.** A read-only review output, not a rebuild step, that computes the two
...
| `n_socs_used` | INTEGER | 0 | How many SOCs back the labor metrics (0 to 3) |
| `no_labor_data` | BOOLEAN | 0 | TRUE when `n_socs_used = 0`: no crosswalk-backed labor signal at all (4/126 TRUE) |
...
| `matched_soc_codes` | TEXT | 4 | Comma-separated top-3-or-fewer SOC codes, same NULL pattern as `employment_weighted_openings` |

### `shortlist_review_clustered` (`data/derived/shortlist_review_clustered.csv`)

Built by `scripts/cluster_shortlist_titles.py` (Python; near-duplicate title detection
is a text-similarity problem the pipeline hands off to Python for this one step, then
rejoins to SQL in `sql/10`). **Grain: same 126 rows as `shortlist_review`**, with two
columns added identifying which of the 61 distinct real-world program candidates (27
multi-member clusters plus 34 singles) each row belongs to.
...
| `algo_cluster_size` | INTEGER | 0 | Row count sharing that `algo_cluster_id` (2 to 13 for clusters, 1 for singles) |
...
Built by `sql/11_dashboard_extracts.sql` from
...
| `score` | numeric | 0 | 0 to 100 composite score |
...
| `year` | integer | 0 | IPEDS reporting year, 2012 to 2024 |
