# Phase 4: dashboard design spec for the new-program opportunity dashboard

*The design for the Phase 4 Tableau dashboard, drafted during Phase 4 planning
(2026-08-08/09) and checked against two independent reads of the same design
literature, *Storytelling with Data* and *The Big Book of Dashboards*: one via
AI-assisted research, one via a parallel NotebookLM read of the same two books. Both
converged on close to the same structure without coordinating, which is most of why
it's locked here instead of argued over further. The Tableau build in the next step
...
out of 1,254 real candidates. This dashboard shows which one is strongest, why, and how
...
Built around Shneiderman's mantra (overview first, zoom and filter, details on demand)
instead of four equally-weighted boxes with no hierarchy between them.
...
completions trend line, 2012 to 2024, finalists only; durability over time doesn't show
up anywhere else on the dashboard.
...
**A School/College filter.** Real idea, raised back in Phase 3 (noticing three of the
five finalists are tech-adjacent), but it needs a manual Vanderbilt college/school
mapping for all 1,254 candidates that was never collected; new data work, not a
dashboard decision. Stays parked.
