...
Go/Test/Pass score. This is a findings report, not a locked decision: every number below
is independently reproducible from `data/derived/derived_candidates.csv`, and Pranish is
verifying it in a separate Python notebook before any of this is implemented in SQL.*
...
- Business Administration and Management, General (CIP 52.0201) has 97,150 completions
  — nearly 3x the #2 program (Social Work, 44.0701, at 34,458) and ~1,500x the median
...
(64.5 completions) would score ≈0.07/100 — nearly every program gets crushed near zero
by one outlier. **Rejected** for the two high-skew metrics.

**Percentile rank** — each candidate's percentile position in its own metric's
distribution (0–100), independent of magnitude. Robust to outliers by construction,
...
`in_bls_top30_flag` is binary and skips this step entirely — it becomes a straight
0/100 component in the weighted sum.
...
## 3. Composite weight proposal — Pranish's decision to approve, not Claude's to lock
...
percentages are often a small-base artifact rather than real momentum.
...
2. **Partial reweight** — drop the missing metric(s) for that row and rescale the
   ...
   uses only real signal. Risk: a candidate scored on only 2–4 of 5 components could
...
Nothing in sections 2–4 is implemented in SQL. This report is Claude's analysis and
