...
> signal it would have provided (full detail in `decisions-log-detailed.md`). The TAM /
> capture-rate formula was never used, because the locked scoring reads IPEDS completions
> straight off as the market-size metric, with no capture rate to assume. The
> student-demand signal itself stayed: IPEDS completions is the largest single input to
> the score, about 45% of it. Competitor pricing is deferred to future scope. For what was
> actually collected and used, see `data-sources.md` and `README.md`.

## 1. BLS: labor demand (bls.gov)

| Dataset | What it gives | Format | Use |
|---------|---------------|--------|-----|
| Occupational Outlook Handbook (OOH) | Per-occupation median pay, 10-yr growth %, jobs, entry education | Web page (scrape) | Already pulled for 12 occupations, saved to `data/raw/bls_labor_demand.csv` |
| **Employment Projections, Table 1.10** (`bls.gov/emp/tables/occupational-separations-and-openings.htm`) | **Annual average occupational openings**, the real "how many jobs open up a year" number: separations plus growth, not just net employment change, for every SOC occupation at once | HTML table (confirmed scrapable) **or** one XLSX download (`bls.gov/emp/ind-occ-matrix/occupation.xlsx`) covering ~800 occupations in one file | Better labor-demand metric than OOH's "employment change" since it captures replacement demand too. One pull covers every candidate, no per-occupation scraping needed |
...
for the same CIP can differ, and this cube can't tell them apart. It can't be the
primary online-competitor-pricing source on its own; treat it as a cross-check alongside
a delivery-mode-aware source, like per-field online rankings.
...
| Google Trends | National search interest by term, 5-yr trend | `pytrends` / manual export | Was the planned `student_demand.csv` search-interest layer in this project's original scope, later cut (see `docs/decisions-log-detailed.md`, Step 6) |
...
## Recommendation: best 2 to 3 datasets to build on

*Superseded. This was a Phase 1 exploratory recommendation. See `data-sources.md` and
`decisions-log-detailed.md` for what was actually collected and why.*
...
This leans the pipeline toward two free structured APIs doing the heavy lifting, with
scraping only for the one thing APIs can't give: program rankings. That's a more
practical build for a project on a 2 to 3 session timebox.

## Claude Code exploration prompt

Lives in `docs/ai-prompts-log.md`, prompt **1.1: Map the data landscape** (kept with all
the other prompts so they're in one place).
