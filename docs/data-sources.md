...
| `competitors_pricing.csv` | Competitor program pages (Firecrawl) + IPEDS | | **Cut 2026-08-01** | Dropped entirely to protect the screening-call deadline, not just deferred — no raw file exists and none is planned before Aug 3. See `decisions-log.md`. |
| `student_demand.csv` | Google Trends + IPEDS completions | | **Cut 2026-08-01** | Same as above — dropped entirely. IPEDS multi-year completions (already collected) covers the demand-trend signal on its own. |
| `docs/planning/CIP_Codes.pdf` | Vanderbilt Registrar's official CIP code list ("Reviewed August 2025") | Tracked and public in `docs/planning/` — deliberate (Pranish, 2026-08-08): it's not treated as sensitive, so it goes in the public planning folder like everything else there. (This row previously said "not tracked in git — university-internal PDF," reflecting an earlier, since-superseded assumption.) | 2026-08-01 | Used to assign/verify `cip2020_code` for 13 of Vanderbilt's 15 online Master's/Doctorate programs; found and provided by Pranish directly, not scraped. See `decisions-log.md` for the full mapping and match-rate against the algorithmic shortlist. |
