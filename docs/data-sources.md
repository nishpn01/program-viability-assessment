...
| `competitors_pricing.csv` | Competitor program pages (Firecrawl) + IPEDS | | **Cut 2026-08-01** | Dropped entirely to protect the screening-call deadline, not just deferred. No raw file exists and none is planned before Aug 3. See `decisions-log-detailed.md`. |
| `student_demand.csv` | Google Trends + IPEDS completions | | **Cut 2026-08-01** | Dropped entirely, same reasoning as above. IPEDS multi-year completions (already collected) covers the demand-trend signal on its own. |
| `docs/planning/CIP_Codes.pdf` | Vanderbilt Registrar's official CIP code list ("Reviewed August 2025") | Tracked and public in `docs/planning/`. Deliberate (Pranish, 2026-08-08): it's not treated as sensitive, so it goes in the public planning folder like everything else there. (This row previously said "not tracked in git, university-internal PDF," reflecting an earlier, since-superseded assumption.) | 2026-08-01 | Used to assign/verify `cip2020_code` for 13 of Vanderbilt's 15 online Master's/Doctorate programs; found and provided by Pranish directly, not scraped. See `decisions-log-detailed.md` for the full mapping and match-rate against the algorithmic shortlist. |
...
Considered in early scoping; the project's actual scoring method (percentile-rank
composite, locked 2026-08-05) doesn't use a TAM/capture-rate calculation, so this was
never filled in. Left here as a record of an approach that was considered and dropped,
not an open task.
